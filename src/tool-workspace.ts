...
import { prepareIsolatedWorkspace } from "./worktrees.js";
...
export function prepareWorkspace(input: unknown, context: ToolContext): { workspace: WorkspaceHandle; warnings: string[] } {
  ...
  if (value.isolated === true) {
    if (value.baseBranch !== undefined && (typeof value.baseBranch !== "string" || !isSafeBranch(value.baseBranch))) throw nodeError("invalid_request", "Invalid base branch");
    return { workspace: prepareIsolatedWorkspace(context, value.runId, value.baseBranch as string | undefined), warnings: [] };
  }
...
export function finalizeWorkspace(input: unknown, context: ToolContext): { committed: boolean; sha?: string } {
  ...
  if (workspace.root !== context.root || gitRaw(context, ["rev-parse", "--abbrev-ref", "HEAD"]).stdout.trim() !== workspace.branch) {
    throw nodeError("policy_denied", "Workspace root or branch does not match the active run", 403);
  }
  ...
  if (value.publishRemote === true) {
    ...
    const permission = checkPublishAllowed(context.policy);
    if (!permission.allowed) throw nodeError("policy_denied", permission.reason ?? "Publishing is not allowed by policy", 403);
    const push = gitPush(context, ["origin", `refs/heads/${workspace.branch}:refs/heads/${workspace.branch}`], 120_000);
...
export function gitRaw(context: ToolContext, args: string[], timeout = 30_000): GitResult {
...
export function spawnGit(root: string, args: string[], timeout: number): GitResult {
