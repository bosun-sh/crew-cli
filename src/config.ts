...
export function loadConfig(env: NodeJS.ProcessEnv = process.env): Config {
  ...
  const auditDir = env.CREW_AUDIT_DIR ? resolve(env.CREW_AUDIT_DIR) : resolve(homedir(), ".crew", "audit");
  ...
  return {
    ...
    allowPublish: parseBoolean(env.CREW_ALLOW_PUBLISH, true, "CREW_ALLOW_PUBLISH"),
    auditDir,
    stateDir,
    auditMaxBytes,
    auditRetentionDays,
...
export function secureUrl(value: string): string {
  if (value.length > 2_048) throw nodeError("invalid_request", "Crew CLI control-plane URL is too long", 1);
  const url = new URL(value);
  const loopback = url.hostname === "localhost" || url.hostname === "127.0.0.1" || url.hostname === "[::1]";
  if (url.protocol !== "https:" && !(url.protocol === "http:" && loopback)) {
    throw nodeError("invalid_request", "Crew CLI control-plane URLs require HTTPS except on loopback", 1);
  }
  if (url.username || url.password) {
    throw nodeError("invalid_request", "Crew CLI control-plane URLs must not contain credentials", 1);
  }
