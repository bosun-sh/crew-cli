...
export function readJobResult(stateDir: string, jobId: string): StoredJobResult | undefined {
  ...
  try {
    ...
    // Do not reinterpret lost evidence as permission to repeat a potentially mutating command.
    return { error: "Stored job result is unreadable; execution outcome is unknown and cannot be replayed safely." };
  } finally {
