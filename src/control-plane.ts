...
import { readJobResult, StoredJobResult, writeJobResult } from "./job-store.js";
...
import { syncNextPullRequest } from "./review-sync.js";

const MAX_RESPONSE_BYTES = 1_000_000;
export const HEARTBEAT_MS = 30_000;
const REQUEST_TIMEOUT_MS = 30_000;
const INTERRUPTED = "Execution was interrupted before its result was persisted; inspect the workspace before retrying this operation.";
  ...
  while (!signal.aborted) {
    try {
      ...
      if (repositories) {
        ...
        await syncNextPullRequest(config, signal, fetchFn).catch(() => dependencies.onError?.("Crew CLI review synchronization failed; it will retry."));
      }
      ...
      try {
        ...
        dependencies.onError?.(`Crew CLI received a malformed job envelope${jobId ? ` (${jobId})` : ""}: ${toNodeError(error).message}`);
        failures = 0;
      ...
      if (job) {
        ...
        if (stored?.error === INTERRUPTED && replayableJob(job)) stored = undefined;
        if (!stored) {
          try {
            // A crash after a side effect must not silently execute the same command again.
            writeJobResult(config.stateDir, job.id, { error: INTERRUPTED });
        ...
        // Keep acknowledged results: a later redelivery must not repeat a mutation.
      }
      failures = 0;
...
export function replayableJob(job: Pick<NodeControlJob, "tool" | "input">): boolean {
  if (["read-repo-file", "list-repo-files", "git-status", "git-diff", "read-cumulative-diff", "collect-safety-findings", "publish-workspace"].includes(job.tool)) return true;
  return job.tool === "prepare-workspace" && Boolean(job.input && typeof job.input === "object" && "isolated" in job.input && job.input.isolated === true);
}
...
function safeError(error: unknown): string {
  return isHttpError(error) ? `Crew CLI control-plane request failed (${error.status})` : "Crew CLI control-plane request failed";
}
