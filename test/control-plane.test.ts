...
import { replayableJob, startControlPlane } from "../src/control-plane.js";
...
test("control plane retries a durable result without executing a redelivered job twice", async () => {
  ...
  const fetchFn = async (input: string | URL | Request): Promise<Response> => {
    ...
    if (url.endsWith("/result")) {
      ...
      if (deliveries === 3) { delivered(); queueMicrotask(() => void control.stop()); }
  ...
  expect(readJobResult(config.stateDir, "job-1")).toEqual({ result: { done: true } });
});

test("interrupted command mutations require attention while isolated preparation can reconcile", () => {
  expect(replayableJob({ tool: "run-command", input: { argv: ["npm", "test"] } })).toBe(false);
  expect(replayableJob({ tool: "prepare-workspace", input: {} })).toBe(false);
  expect(replayableJob({ tool: "prepare-workspace", input: { isolated: true } })).toBe(true);
  expect(replayableJob({ tool: "read-repo-file", input: {} })).toBe(true);
});
...
test("control plane rejects oversized chunked responses before JSON parsing", async () => {
  ...
  await failure;
  await control.stop();
  expect(message).toBe("Crew CLI control-plane request failed");
});
