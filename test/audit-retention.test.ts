...
describe("audit log rotation and retention", () => {
  test("rolls the live log to a timestamped file once it reaches auditMaxBytes", () => {
    ...
    expect(entries).toContain("crew-cli-audit.jsonl");
    const rotated = entries.filter((entry) => entry.startsWith("crew-cli-audit-") && entry.endsWith(".jsonl"));
    expect(rotated.length).toBe(1);

    const liveContent = readFileSync(join(config.auditDir, "crew-cli-audit.jsonl"), "utf8");
  ...
  test("deletes rotated files older than auditRetentionDays but keeps recent ones and the live log", () => {
    ...
    const oldRotated = join(config.auditDir, "crew-cli-audit-2000-01-01T00-00-00-000Z.jsonl");
    const recentRotated = join(config.auditDir, "crew-cli-audit-2999-01-01T00-00-00-000Z.jsonl");
    ...
    const entries = readdirSync(config.auditDir);
    expect(entries).not.toContain("crew-cli-audit-2000-01-01T00-00-00-000Z.jsonl");
    expect(entries).toContain("crew-cli-audit-2999-01-01T00-00-00-000Z.jsonl");
    expect(entries).toContain("crew-cli-audit.jsonl");
  });
});
