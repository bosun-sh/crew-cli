...
The worker is intentionally allowed to change explicitly enabled repositories. It is not a sandbox for hostile binaries already present in those repositories. Run it on a dedicated Linux host or VM, connect only approved repositories, and scope Git credentials narrowly. Crew Cloud and the configured model provider can receive context requested by approved work; The worker does not provide end-to-end confidentiality from those services.
